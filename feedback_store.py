...
def save_feedback(player_name: str, text: str, git_sha: str = "") -> Path:
    ...
    Sanitises the player name so it is safe to use in a filename
    (replaces any non-alphanumeric characters with underscores).

    Returns the Path of the file that was written.
    """
    # Create the feedback directory if it doesn't already exist.
    FEEDBACK_DIR.mkdir(parents=True, exist_ok=True)

    # Build a filename like: 2026-04-29_22-15-30_Zac.txt
    ts   = time.strftime("%Y-%m-%d_%H-%M-%S")
    # Keep only alphanumeric, dash, and underscore characters in the name.
    safe = "".join(
    ...
    # Build the file content and write it out.
    content = (
...
def list_feedback() -> list:
    ...
    """
    # If no feedback has ever been submitted, the directory won't exist yet.
    if not FEEDBACK_DIR.exists():
        return []

    results = []

    # Sort in reverse so the most recently submitted file appears first.
    for f in sorted(FEEDBACK_DIR.glob("*.txt"), reverse=True):
        try:
            ...
            # Line 4 is the first line of the actual feedback text (after the blank line).
            preview = lines[4][:60] if len(lines) > 4 else ""
            results.append((f.name, player, ts, preview))
        except Exception:
            # Skip any file we can't parse cleanly rather than crashing.
            pass

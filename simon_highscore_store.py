...
(seq_length), with total rounds completed as a tiebreaker. The top
TOP_N results are kept on disk.
...
class SimonHighscoreStore:
    ...
    Rankings are determined by:
      ...
      2. Total rounds completed as a tiebreaker (higher is better).
    """
    ...
    def submit(self, player_name: str, score: int, seq_length: int) -> tuple:
        ...
        score       -- total rounds the player completed
        seq_length  -- longest correct gesture chain reached
        ...
        # Find the rank. Try identity match first, then match by all three fields.
        rank = next(
    ...
    def _load(self):
        ...
        Starts with an empty list if the file is missing or corrupt --
        the file will be created on the next save.
        """
        try:

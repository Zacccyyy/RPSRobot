...
interface is identical so the rest of the app only needs to swap
SerialBridge → BLEBridge.
...
class BLEBridge:
    ...
    The game loop is synchronous but bleak is async, so this class runs a
    private asyncio event loop in a background daemon thread.  All public
    methods submit coroutines to that loop and wait for the result, which
    keeps the public API simple and synchronous.
    ...
    def __init__(self, log_limit=50):
        ...
        self.bridge_type = "BLE"  # used by HardwareTestController to know the active mode

        # BLE connection state.
        self._client      = None   # BleakClient object when connected
        self._device_name = None
        self._device_addr = None
        self._connected   = False

        # Partial line buffer — incoming BLE data can arrive in chunks.
        self._read_buffer = ""

        # Complete response lines waiting to be read by the main thread.
        self._response_queue = deque(maxlen=100)

        # Lock protecting all shared state between the main thread and the
        # background BLE event loop thread.
    ...
    def _run_async(self, coro, timeout=10.0):
        ...
        This bridges the gap between the synchronous caller and async bleak.
    ...
    def scan_devices(self, timeout=SCAN_TIMEOUT) -> list[dict]:
        """
        Scan for nearby BLE devices and return a list of discovered ones.

        The ESP32 is identified by its Nordic UART service UUID (more reliable
        than name matching, because macOS hides device names until after the
        first connection).  ESP32 devices are sorted first in the result.
        ...
        if not BLEAK_AVAILABLE:
            ...
            return []
        try:
            # Allow extra time so the async scan can finish cleanly.
            return self._run_async(self._async_scan(timeout), timeout=timeout + 2)
        except Exception as e:
            print(f"[BLE] Scan error: {e}")
            return []

    async def _async_scan(self, timeout) -> list[dict]:
        """
        Async implementation of device discovery.

        return_adv=True gives us advertisement data including the list of
        service UUIDs — that's how we spot the ESP32 on macOS where device
        names aren't visible until after the first connection.
        """
        ...
        for addr, (device, adv) in discovered.items():
            ...
            is_esp  = NUS_SERVICE_UUID in uuids or ESP32_DEVICE_NAME in name
            display = "RPS Robot (ESP32)" if is_esp else (name or "Unknown")

            result.append({"name": display, "address": device.address, "is_esp": is_esp})

        # Sort so the ESP32 appears at the top, then alphabetically within each group.
    ...
    def connect(self, address, device_name="") -> bool:
        """
        ...
        try:
            # Allow 15 seconds — BLE connection setup can be slow on first pair.
    ...
    async def _async_connect(self, address, device_name):
        """
        Async implementation of the BLE connection sequence.

        After connecting we subscribe to RX characteristic notifications so
        incoming data from the ESP32 automatically calls _on_notify() without
        us having to poll.
        """
        try:
    ...
    def _on_disconnect(self, client):
        ...
        Clears connection state so the rest of the app knows we're offline.
        """
    ...
    def disconnect(self):
        """Disconnect from the BLE device gracefully and clear all state."""
        if self._client is not None:
            try:
                self._run_async(self._async_disconnect(), timeout=5)
            except Exception:
                pass  # ignore errors on disconnect — we're cleaning up anyway
    ...
    async def _async_disconnect(self):
        """Async implementation: send a BLE disconnect if the client is still alive."""
    ...
    def send_command(self, action) -> bool:
        ...
        for custom strings.  Appends a newline because the ESP32 firmware reads
        line-by-line.
    ...
    def _on_notify(self, sender, data):
        ...
        Incoming bytes are appended to a line buffer.  Complete lines (ending in
        newline) are extracted and pushed onto the response queue for the main
        thread to consume via read_response().
        """
        text = data.decode("utf-8", errors="replace")

        # Collect lines to print BEFORE releasing the lock, to avoid holding
        # the lock while doing I/O (which could cause a subtle deadlock).
        lines_to_print = []
        ...
        # Print after releasing the lock.
    ...
    def list_ports(self) -> list[str]:
        """
        Scan for BLE devices and return display strings compatible with
        SerialBridge.list_ports().

        Format: "<name> | <address>" for each discovered device.
        """
    ...
    def get_status_summary(self) -> dict:
        ...
        Used by the hardware test UI.  Matches the SerialBridge interface.
        """
        if not BLEAK_AVAILABLE:
            return {

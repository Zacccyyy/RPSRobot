...
# How many scores to keep in the leaderboard.
TOP_N = 10
...
class ReflexHighscoreStore:
    ...
    def submit(self, player_name: str, score: int, avg_rt_ms: int) -> tuple:
        ...
        score       : int  -- number of correct throws in the session
        avg_rt_ms   : int  -- average reaction time in milliseconds

        Returns
        -------
        (is_new_highscore, rank_1indexed)
        ...
        # Snapshot the current best before we add the new entry.
        ...
        # Trim to the top TOP_N entries.
        self._scores = self._scores[:TOP_N]

        # Find this entry's rank.  We match by object identity first (exact
        # same dict) and fall back to matching all three identifying fields
        # in case the list was rebuilt by sort.
    ...
    def _load(self):
        """
        Load scores from disk.  If the file is missing or corrupt we start
        with an empty list rather than crashing — the file will be created
        on the next save.
        """
        try:
